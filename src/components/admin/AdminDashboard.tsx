...
import { Briefcase, FileText, Inbox, Layers, Megaphone, Wrench } from 'lucide-react'
...
interface DashboardStats {
  ...
  newEnquiries: number
  newCampaignApplications: number
...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: stats?.newCampaignApplications ?? 0,
      icon: Megaphone,
    ...
    },
    {
      ...
      value: stats?.newCareerApplications ?? 0,
      icon: Briefcase,
    ...
    { label: 'Active Jobs', value: stats?.activeCareers ?? 0, icon: Briefcase, href: '/admin/careers', color: 'text-teal-600 bg-teal-50' },
  ...
  return (
    <div>
      <AdminHeader
        title="Dashboard"
        description="Welcome to the Reliable Company CMS — manage all content and form submissions here."
        action={
      ...
      <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-semibold text-gray-900">Campaign Applications</h2>
            <Link href="/admin/campaign-applications" className="text-sm text-primary-600 hover:underline">
              View all
            </Link>
          </div>
          <ul className="space-y-3">
            {(stats?.recentCampaignApplications ?? []).map((item) => (
              <li key={item._id} className="flex items-center justify-between gap-3 border-b border-gray-100 pb-3 last:border-0 last:pb-0">
                <div>
                  <p className="text-sm font-medium text-gray-900">{item.fullName}</p>
            ...
            {!stats?.recentCampaignApplications?.length && (
              <li className="text-sm text-gray-500">No campaign applications yet.</li>
            )}
          </ul>
        </section>
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-semibold text-gray-900">Career Applications</h2>
            <Link href="/admin/career-applications" className="text-sm text-primary-600 hover:underline">
              View all
            </Link>
          </div>
          <ul className="space-y-3">
            {(stats?.recentCareerApplications ?? []).map((item) => (
              <li key={item._id} className="flex items-center justify-between gap-3 border-b border-gray-100 pb-3 last:border-0 last:pb-0">
                <div>
                  <p className="text-sm font-medium text-gray-900">{item.fullName}</p>
            ...
            {!stats?.recentCareerApplications?.length && (
              <li className="text-sm text-gray-500">No career applications yet.</li>
            )}
          </ul>
        </section>
      </div>
